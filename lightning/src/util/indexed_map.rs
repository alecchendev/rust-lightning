...
impl<K: Clone + Hash + Ord + std::fmt::Debug, V> IndexedMap<K, V> {
	...
	pub fn remove(&mut self, key: &K) -> Option<V> {
		...
		if let Some(_) = ret {
			let idx = self.keys.binary_search(key).ok().expect("map and keys must be consistent");
...
pub struct Range<'a, K: Hash + Ord, V> {
	inner_range: std::slice::Iter<'a, K>,
	map: &'a HashMap<K, V>,
}
...
impl<'a, K: Hash + Ord, V> VacantEntry<'a, K, V> {
	...
	pub fn insert(self, value: V) -> &'a mut V {
		let idx = self.keys.partition_point(|k| k < &self.key);
		self.keys.insert(idx, self.key);
...
impl<'a, K: Hash + Ord, V> OccupiedEntry<'a, K, V> {
	...
	pub fn remove_entry(self) -> (K, V) {
		...
		let idx = self.keys.binary_search(&res.0).expect("map and keys must be consistent");
		self.keys.remove(idx);
		res
	}
