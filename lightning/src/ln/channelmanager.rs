		...
		let mut has_update = false;
		{
			...
			for (_, peer_state_mutex) in per_peer_state.iter() {
				...
				for (_, chan) in peer_state.channel_by_id.iter_mut() {
					if let Some(closing_ready_event) = chan.maybe_get_closing_ready() {
						self.pending_events.lock().unwrap().push_back((closing_ready_event, None));
						has_update = true;
					}
				}
			}
