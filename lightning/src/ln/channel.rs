...
pub(super) struct ChannelContext<Signer: ChannelSigner> {
	...
	// If our counterparty initiates a splice, we keep track of the amount they want to splice
	...
	splice_feerate_per_kw: Option<u32>,
...
impl<Signer: WriteableEcdsaChannelSigner> Channel<Signer> {
	...
	pub fn set_splice_feerate_per_kw(&mut self, feerate_per_kw: Option<u32>) {
		self.context.splice_feerate_per_kw = feerate_per_kw;
	}
			...
			SP::Target: SignerProvider
{
	fn read<R : io::Read>(reader: &mut R, args: (&'a ES, &'b SP, u32, &'c ChannelTypeFeatures)) -> Result<Self, DecodeError> {
		...
		let mut splice_feerate_per_kw = None;
		...
		read_tlv_fields!(reader, {
			...
			(47, splice_feerate_per_kw, option),
		});
